...
The app progresses through four phases driven by a single `wl_display_dispatch` event loop in `main()`:
...
3. **PHASE_RECAPTURE** — triggered by right-click. All overlay surfaces are committed with blank (transparent) buffers and the cursor is hidden, then fresh capture sessions are started on every output. Because Wayland requests are ordered on the wire, the blank commits arrive at the compositor before the capture frame request. Once all captures complete (`captures_pending` → 0) the cursor is restored and phase returns to PICK. The pixel buffers are **reused** (not reallocated) on recapture.
4. **PHASE_CLIPBOARD** — on left-click, the hex string is written to a `wl_data_source` and offered via `wl_data_device.set_selection`. All overlay and cursor surfaces are destroyed. The event loop blocks until `wl_data_source.send` (paste) or `wl_data_source.cancelled` (selection replaced) fires, then exits 0.
...
```c
typedef enum {
    PHASE_CAPTURE, PHASE_PICK, PHASE_RECAPTURE, PHASE_CLIPBOARD, PHASE_DONE
} Phase;
...
- `commit_blank_overlay(Output*)` — memsetting overlay to transparent and committing; used during recapture to hide the preview before capture
...
- `start_recapture(App*)` — commits blank to all overlays, hides cursor, sets `PHASE_RECAPTURE`, starts fresh capture sessions on all outputs, flushes
- `pointer_button` — right-click → `start_recapture`; left-click → destroy overlays/cursor, create data_source, set clipboard selection, set `PHASE_CLIPBOARD`; uses button-press `serial` (not stale enter serial)
...
- `capture_frame_ready` — destroys frame/session/source; decrements `captures_pending`; on RECAPTURE: restores cursor + redraws overlay; on initial CAPTURE: calls `start_pick_phase`
- `capture_session_done` — allocates pixel buffer if not already allocated (reuses on recapture), creates frame, attaches + damages + fires capture in one step
...
- `main` — 3 roundtrips; validates globals; creates data_device; starts captures; runs merged CAPTURE+PICK+RECAPTURE event loop; checks `PHASE_DONE` (Escape → exit 0) before `PHASE_CLIPBOARD` check; runs clipboard loop; cleanup (munmap all buffers)
...
- `capture_session_done` must **not** reallocate the pixel buffer on recapture (same output, same dimensions); check `!out->pixels` first
